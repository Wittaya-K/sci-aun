<template>
  <AdminLayout :allCriteria="allCriteria" :activeCriteriaId="criteria.id">
    ...
    <div class="flex items-center gap-2 mb-6 text-sm text-gray-400">
      ...
      <span>/</span>
      <span class="font-medium text-gray-700">{{ subCriteria.number }}</span>
    </div>

    <!-- Header -->
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-lg font-semibold text-gray-900">{{ subCriteria.number }}</h1>
        <p class="text-sm text-gray-500 mt-0.5 max-w-2xl">{{ subCriteria.title }}</p>
      </div>
