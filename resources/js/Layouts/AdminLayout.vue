<template>
  <div class="flex flex-col min-h-screen bg-gray-50">
    ...
    <!-- Top bar -->
    <header class="sticky top-0 z-10 flex items-center justify-between px-6 py-3 bg-white border-b border-gray-200">
      <div class="flex items-center gap-3">
        <img src="/images/psu-logo.jpg" alt="PSU" class="w-auto h-8" />
        <div>
          <div class="text-xs leading-none text-gray-400">คณะวิทยาศาสตร์ มหาวิทยาลัยสงขลานครินทร์</div>
          <div class="text-sm font-semibold leading-tight text-primary-500">AUN-QA Admin</div>
        </div>
      </div>
      <div class="flex items-center gap-4">
        <a :href="route('aun.show', 1)" target="_blank"
            class="flex items-center gap-1 text-xs text-gray-400 transition-colors hover:text-primary-500">
        ...
        <span class="text-xs text-gray-400">{{ $page.props.auth.user?.name }}</span>
    ...
    <div class="flex flex-1 overflow-hidden">
      <!-- Sidebar -->
      <aside class="flex-shrink-0 w-56 overflow-y-auto bg-white border-r border-gray-200">
        <nav class="py-4">
          <div class="px-4 mb-2 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">
            จัดการ AUN-QA
          </div>
